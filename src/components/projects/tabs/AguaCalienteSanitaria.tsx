...
const AguaCalienteSanitaria = () => {
    ...
    const acsDataList = Object.entries(occupancyResult.constant?.atributs?.tipo_de_ocupacion_acs || {}).map(
        ([tipo, consumo]) => ({ tipo, cantidad: 0, consumo: Number(consumo) })
    );
    ...
    const [consumoEnergiaPrimariaACS, setConsumoEnergiaPrimariaACS] = useState<number>(3.576);
    ...
    const [sistControl, setSistControl] = useState<string | number>('Automático base a T° agua');
    const [tAcs, setTAcs] = useState<number>(0);

    const [combustibleOptions, setCombustibleOptions] = useState<{ value: string; label: string; fep: number }[]>([]);
    const [rendimientoOptions, setRendimientoOptions] = useState<{ value: string; label: string; rendimiento: number }[]>([]);
    const [sistDistribucionOptions, setSistDistribucionOptions] = useState<{ value: string; label: string; distribucion: number }[]>([]);
    const [sistControlOptions, setSistControlOptions] = useState<{ value: string; label: string; control: number }[]>([]);

    const [energySourceConsumption, setEnergySourceConsumption] = useState<{ [key: string]: number }>({});
    useEffect(() => {
        if (result.constant?.atributs?.consumos_por_fuente_de_energia) {
            setEnergySourceConsumption(result.constant?.atributs?.consumos_por_fuente_de_energia)
        }
    }, [])
    ...
    useEffect(() => {
        if (result.constant?.atributs) {
            const { combustibles, rendimiento_acs, distribucion_acs, control_acs } = result.constant.atributs;

            setCombustibleOptions(
                combustibles.map((item: any) => ({ value: item.code, label: item.name, fep: item.fep }))
            );
            setRendimientoOptions(
                rendimiento_acs.map((item: any) => ({ value: item.code, label: item.name, rendimiento: item.value }))
            );
            setSistDistribucionOptions(
                distribucion_acs.map((item: any) => ({ value: item.code, label: item.name, distribucion: item.value }))
            );
            setSistControlOptions(
                control_acs.map((item: any) => ({ value: item.code, label: item.name, control: item.value }))
            );

            // Set default values only if they are not already set
            if (!combustible && combustibles.length > 0) setCombustible(combustibles[0].code);
            if (!rendimiento && rendimiento_acs.length > 0) setRendimiento(rendimiento_acs[0].code);
            if (!sistDistribucion && distribucion_acs.length > 0) setSistDistribucion(distribucion_acs[0].code);
            if (!sistControl && control_acs.length > 0) setSistControl(control_acs[0].code);
        }
    }, [result.constant]);
    ...
    useEffect(() => {
        const rendimientoValue = getValueFromValue(rendimientoOptions, rendimiento, 'rendimiento');
        const sistDistribucionValue = getValueFromValue(sistDistribucionOptions, sistDistribucion, 'distribucion');
        const sistControlValue = getValueFromValue(sistControlOptions, String(sistControl), 'control');
        const combustibleFep = getValueFromValue(combustibleOptions, combustible, 'fep');

        if (rendimientoValue && sistDistribucionValue && sistControlValue && combustibleFep) {
            const calculatedConsumoEnergiaPrimariaACS =
                (combustibleFep * demandaACS) / (sistControlValue * sistDistribucionValue * rendimientoValue);
            setConsumoEnergiaPrimariaACS(parseFloat(calculatedConsumoEnergiaPrimariaACS.toFixed(3)));
        }
    ...
    const handleInputChange = (index: number, field: keyof TableRow, value: string) => {
        ...
        if (field === 'tipo') {
            const selectedOption = acsDataList.find(option => option.tipo === value);
            updatedData[index].consumo = selectedOption ? selectedOption.consumo : 0;
        }
        updatedData[index] = {
            ...updatedData[index],
            [field]: field === 'cantidad' || field === 'consumo' ? parseFloat(value) || 0 : value
        };
        setTableData(updatedData);
    };

    const addRow = () => {
        const newRow = acsDataList.length > 0 ? acsDataList[0] : { tipo: '', cantidad: 0, consumo: 0 };
    ...
    const handleCombustibleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const selectedOption = combustibleOptions.find(option => option.value === selectedCode);
        if (selectedOption) {
        ...
        <Row className="mb-4">
            <Col md={12}>
                <Card className="shadow">
                    <Card.Body>
                        <Card.Title className="text-center text-dark">Agua Caliente Sanitaria</Card.Title>
                        <Table bordered responsive className="table-sm mb-0">
                            <thead className="table-light">
                                <tr>
                                    <th>Tipo de ocupación para sistema ACS</th>
                                    <th>Cantidad personas</th>
                                    <th><span className="fw-bold">[l/pers-día]</span></th>
                                    <th>Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm={4} className="text-end">
                                    <div className="form-control-plaintext fw-bold">{demandaACS}</div>
                                </Col>
                                <Col sm="auto" className="align-self-center">
                                    <span>[kWh]</span>
                                </Col>
                        ...
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm="auto" className="align-self-center">
                                    <span>{getValueFromValue(combustibleOptions, combustible, 'fep')}</span> <span>[FEP]</span>
                                </Col>
                            </Form.Group>
                        </div>
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm="auto" className="align-self-center">
                                    <span>{getValueFromValue(rendimientoOptions, rendimiento, 'rendimiento')}</span> <span>[FEP]</span>
                                </Col>
                            </Form.Group>
                        </div>
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm={4} className="text-end">
                                    <Form.Select
                                        ...
                                        onChange={(e) => setSistDistribucion(String(e.target.value))}
                                    >
                                        <option value="">Seleccione</option>
                                ...
                                <Col sm="auto" className="align-self-center">
                                    <span>{getValueFromValue(sistDistribucionOptions, sistDistribucion, 'distribucion')}</span> <span>[FEP]</span>
                                </Col>
                            </Form.Group>
                        </div>
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm="auto" className="align-self-center">
                                    <span>{getValueFromValue(sistControlOptions, String(sistControl), 'control')}</span> <span>[FEP]</span>
                                </Col>
                            </Form.Group>
                        </div>
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm={4} className="text-end">
                                    <div className="form-control-plaintext fw-bold">{consumoACS}</div>
                                </Col>
                                <Col sm="auto" className="align-self-center">
                                    <span>[kWh]</span>
                                </Col>
                        ...
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm={4} className="text-end">
                                    <div className="form-control-plaintext fw-bold">{consumoEnergiaPrimariaACS}</div>
                                </Col>
                                <Col sm="auto" className="align-self-center">
                                    <span>[kWh]</span>
                                </Col>
                        ...
                        <div className="mt-3">
                            <Form.Group as={Row} className="align-items-center">
                                ...
                                <Col sm={4} className="text-end">
                                    <div className="form-control-plaintext fw-bold">{co2eqEnergiaPrimaria}</div>
                                </Col>
                                <Col sm="auto" className="align-self-center">
                                    <span>[kg CO2]</span>
                                </Col>
